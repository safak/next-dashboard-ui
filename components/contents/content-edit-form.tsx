...
import { AppCardProps, ContentCardProps } from '@/types'
...
export const EditContentForm = ({
  ...
  const onSubmit = async (values: z.infer<typeof AddContentSchema>) => {
    try {
      await axios.patch(`/api/content/${id}`, values);
      form.reset();
      router.refresh();
  ...
  return (
    <Form {...form}>
      <form className="ms-1.5 w-[325px] flex flex-col gap-5 pt-5 pb-40"
      ...
      >
        ...
        <Button type="submit" className="w-full" size="lg" disabled={isPending}>{isPending ? <Spinner /> : "Update Content"}</Button>
