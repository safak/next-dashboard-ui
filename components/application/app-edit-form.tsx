...
import { onUpdate } from '@/actions/apps'
...
export const EditAppForm = ({ image, name, status, packagename, description, id, classname
}: AppCardProps) => {
  ...
  const form = useForm<z.infer<typeof AddAppSchema>>({
    ...
    defaultValues: {
      ...
      isStatus: status || undefined
    },
  });

  const onSubmit = async (values: z.infer<typeof AddAppSchema>) => {
    try {
      await axios.patch(`/api/application/${id}`, values);
      form.reset();
      router.refresh();
  ...
  return (
    <Form {...form}>
      <form className="ms-1.5 w-[325px] flex flex-col gap-5 pt-5 pb-16"
      ...
      >
        ...
        <Button type="submit" className="w-full" size="lg" disabled={isPending}>{isPending ? <Spinner /> : "Update App"}</Button>
