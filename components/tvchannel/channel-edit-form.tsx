...
export function EditTvChannel<TData>({ row }: RowActionsProps<TData>) {
  ...
  const form = useForm<z.infer<typeof AddTvChannelSchema>>({
    ...
    defaultValues: {
      tvIcon: row.getValue("tvIcon"),
      tvName: row.getValue("tvName"),
      tvNumber: row.getValue("tvNumber"),
      tvmedia: row.getValue("tvmedia"),
  ...
  const onSubmit = async (values: z.infer<typeof AddTvChannelSchema>) => {
    setIsLoading(true);
    try {
      await axios.patch(`/api/tvchannel/${row.getValue("id")}`, values);
      form.reset();
      router.refresh();
    ...
    <Form {...form}>
      ...
      >
        ...
        <Button type="submit" className="w-full" size="lg" disabled={isLoading}>
          {isLoading ? <Spinner /> : "Update an TV channel"}
        </Button>
      </form>
    </Form>
  )
